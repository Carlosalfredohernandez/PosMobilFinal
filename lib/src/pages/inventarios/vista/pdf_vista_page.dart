...
class PdfVistaPage extends StatefulWidget {
  ...
  List<Inventario> productos = <Inventario>[];
  var total;
  PdfVistaPage({super.key, required this.productos, this.total});

  @override
  State<PdfVistaPage> createState() => _PdfVistaPage(productos: productos, total: total);
}

class _PdfVistaPage extends State<PdfVistaPage> {
  List<DetalleBoleta> detalles = <DetalleBoleta>[];
  List<Inventario> productos = <Inventario>[];
  var total;
  _PdfVistaPage({required this.productos, this.total});
  ...
  Future<Uint8List> generarPdf() async {
    var pdf = pw.Document();
    final headers = ['Documento','Local','Cantidad','Fecha','Proveedor'];
    final data = productos.map((Inventario inventario) =>[
      inventario.nroDocumento ?? '',inventario.local ?? '',inventario.cantidad ?? '', inventario.fecha ?? '', inventario.idProvedor
    ]).toList();
